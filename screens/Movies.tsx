...
const Movies = ({navigation})=>{
  ...
  const obtenerMovies = async () => {
        try {
          const data = await getMovies(query?.replace(' ', '%20'));
          const favoritos = JSON.parse(await AsyncStorage.getItem('favoritos')) || [];
          setFavoritos(favoritos);
          setMovies(data); // Guarda los datos en el estado
  ...
  const handleFavoritos = async(item) => {
    ...
    Alert.alert('Favoritos', `Agregado a favoritos: ${item.show.name}`);
  }
  ...
  const handleEliminarFavoritos = async (id) => {
          ...
          <View style={styles.row}>
              ...
              <View style={styles.infoUSer}>
                ...
                <View style={styles.rowInfo}>
                    {favoritos.find((fav) => fav.show.id === item.show.id) ?
                    <TouchableOpacity
                    style={[styles.button, styles.deleteButton]}
                    onPress={() => handleEliminarFavoritos(item.show.id)}
                    >
                      <Text style={styles.buttonText}>Eliminar de Favoritos</Text></TouchableOpacity>
                      
                      :
                    
                    <TouchableOpacity
                    style={[styles.button, styles.buscarButton]}
                    onPress={() => handleFavoritos(item)}
                    >
                      <Text style={styles.buttonText}>Agregar a Favoritos</Text></TouchableOpacity>}
    ...
    return (
        ...
        <View style={styles.container}>
              <Text style={styles.title}>Programas de TV</Text>
              <View style={styles.row}>
                <TextInput 
                  style={styles.input}
                  placeholder="Buscar una programa de TV"
                  onChangeText={(value) => handleChange(value)}
                />
                <TouchableOpacity
                      style={[styles.button, styles.buscarButton]}
                      onPress={() => obtenerMovies()}
                    >
              ...
              <FlatList
                ...
                ListEmptyComponent={<Text style={styles.emptyText}>No hay progrmas de TV disponibles.</Text>}
              />
        </View>
    );
}
